...
- Options considered: A) Lock the baseline and adopt a standing upgrade rule · B) Choose tools one at a time as each is needed, with no baseline · C) Move to the newest release of everything as soon as it ships
- Chosen: A.
...
- Options considered: A) Valkey 9.1.2 on an exact tag · B) Redis 8 · C) Stay on Redis 7
- Chosen: A.
...
- Options considered: A) 18 now on an exact tag, new volume · B) 18 now, reusing the existing volume via `pg_upgrade` · C) Wait for 19 (final targeted end of October 2026)
- Chosen: A.
...
- Options considered: A) Lint only the migrations a branch adds · B) Lint the whole chain every run · C) Lint the whole chain but exclude every rule that currently fires
- Chosen: A.
...
- Options considered: A) Qwen3-Embedding-0.6B, two derived tables, exact search · B) A larger model (llama-embed-nemotron-8b, KaLM-Gemma3-12B) · C) An embedding API instead of a local model · D) Vectors as columns on `creator` and `campaign`, with an HNSW index from the start
- Chosen: A.
