...
| Item | We run | Current best | Why move | Status | When |
|---|---|---|---|---|---|
| PostgreSQL | **18.6** (`pgvector/pgvector:0.8.6-pg18-trixie`, exact) | Current; **19** after release and its first minor update (final targeted end of October 2026) | 18: native `uuidv7()`, statistics kept across upgrades, current major. **Its asynchronous I/O did NOT make us faster**: measured 23 September, 18.6 and 16.15 are indistinguishable at our data size, because nothing waits on disk yet (`docs/PERFORMANCE.md`). 19: online `REPACK CONCURRENTLY`, parallel autovacuum | **Built** (D-049) | Done |
